...
/**
 * Fix null ordering values in client stories
 * @param {SanityClient} client
 */
export async function fixNullOrdering(client) {
    try {
        // 1. Get all client stories with null displayOrder, ordered by creation date
        /** @type {ClientStory[]} */
        const storiesWithNullOrder = await client.fetch(`
        ...
        const highestOrder = await client.fetch(`
