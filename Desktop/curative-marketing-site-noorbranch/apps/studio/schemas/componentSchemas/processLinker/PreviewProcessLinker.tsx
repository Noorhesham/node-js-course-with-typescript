...
export const PreviewProcessLinker = (props: Props) => {
    ...
    <Box>
      <Flex align="center" gap={3} paddingRight={3}>
        <Box flex={1}>
          {renderDefault({
            ...props,
            title: theme ? `${toTitleCase(theme)} Linker` : 'Process Linker',
            subtitle: tabs?.length ? `${tabs.length} steps` : 'No steps'
          })}
        </Box>
        {componentName && (
          <Badge mode="outline">
          ...
          <Flex gap={2} wrap="nowrap" style={{ width: 'max-content' }}>
            {tabs.map((tab, index) => (
              <Card key={index} padding={3} border>
                <Text size={1}>{tab.heading || `Step ${index + 1}`}</Text>
              </Card>
            ))}
          </Flex>
        </Box>
